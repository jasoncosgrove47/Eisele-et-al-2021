...
##### 0 Import data
x1<-read.table("/Users/almut/Dropbox/Eisele_et_al_EPO/Rscripts_and_data_by_Almut/Filtered_barcoding_data/EPO_160ngml_all_filtered_EPO_6_7_Ctrl_1_2_3.txt", sep="\t", header=TRUE)
...
H3<- cbind(x1[,1],H3)
H4 <- x1[,grep("_6_", colnames(x1))]
H4<- cbind(x1[,1],H4)
H5 <- x1[,grep("_7_", colnames(x1))]
H5<- cbind(x1[,1],H5)

Hmouse<-list(H1, H2, H3, H4, H5)
...
HBimouse<-list(HBi1, HBi2, HBi3, HBi4, HBi5)
...
HBiimouse<-list(HBii1, HBii2, HBii3, HBii4, HBii5)
...
NORM<-gdata::combine(H1, H2, H3, H4, H5)
...
ggtern(data=NORM,aes(Bmean,Emean,Mmean, size=allreadsBEM)) + 
  ...
  labs(x="B",y="E",z="M",title="EPO 160ngml")+
