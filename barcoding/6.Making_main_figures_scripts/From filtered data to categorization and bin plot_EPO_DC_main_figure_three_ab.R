...
# Adapt grep names according to experiments
Hmouse <- lapply(Hmouse, function(x) cbind(x,"Bmean"=rowMeans(x[grep("_B_", names(x))])))
Hmouse <- lapply(Hmouse, function(x) cbind(x,"Dmean"=rowMeans(x[grep("_DC_", names(x))])))
Hmouse <- lapply(Hmouse, function(x) cbind(x,"Emean"=rowMeans(x[grep("_E.P7_", names(x))])))
Hmouse <- lapply(Hmouse, function(x) cbind(x,"Mmean"=rowMeans(x[grep("_M_", names(x))])))
Hmouse <- lapply(Hmouse, function(x) x[,-c(2:(ncol(x)-4))])
...
####### contribution of progenitor classes to DC lineage by Condition

NC<-NORM

NC<- melt(NC[,c("progenitor", "Dmean", "Condition", "source")], id=c("progenitor","Condition", "source"))
...
###### percentage progenitors producing DC
