...
##### 0 Import data
x1<-read.table("/Users/almut/Dropbox/Eisele_et_al_EPO/Rscripts_and_data_by_Almut/Filtered_barcoding_data/EPO_1000ngml+injection_all_filtered_EPO_5_6_7_8_Ctrl_1_2_4.txt", sep="\t", header=TRUE)
...
H4<- cbind(x1[,1],H4)
H5 <- x1[,grep("_6_", colnames(x1))]
H5<- cbind(x1[,1],H5)
H6 <- x1[,grep("_7_", colnames(x1))]
H6<- cbind(x1[,1],H6)
H7 <- x1[,grep("_8_", colnames(x1))]
H7<- cbind(x1[,1],H7)
...
NORM<- within(NORM, Condition[source == "H3"]<- "No EPO")
...
ggtern(data=NORM,aes(Bmean,Emean,Mmean, size=allreadsBEM)) + 
  ...
  labs(x="B",y="E",z="M",title="EPO 1000ngml+injection")+
